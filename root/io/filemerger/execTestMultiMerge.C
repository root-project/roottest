...
int execTestMultiMerge()
{
   ...
   result += testMergedFile("mlz4file1-4.root",406,5395 + lz4default*841 + kIs32bits*2 - kIs32bits*lz4default*16, kIs32bits ? 4 : 2);
   ...
   result += testSimpleFile("hsimple109.root",25000,109,430879 + lz4default*1931 + kIs32bits*14,3 + fastMath*28);
   result += testSimpleFile("hsimple9x2.root",2*25000,9,849980 + lz4default*169479 + kIs32bits*10,9 + fastMath*56);
   result += testSimpleFile("hsimple109x2.root",2*25000,109,849990 + lz4default*1931 + kIs32bits*15,9 + fastMath*52);
