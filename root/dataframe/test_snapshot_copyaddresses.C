...
void write_inputs()
{
   ...
   for (int i = 0; i < nfiles; ++i, ti.x += 2, ti.y += 2) {
      ...
      if (gSystem->AccessPathName(fname.c_str()) == 0) // 0 means file already exists
         continue;
...
void test_snapshot_copyaddresses()
{
   ...
   out_df->Foreach(
      [&](int x, int y) mutable {
         R__ASSERT(x == expected++);
         R__ASSERT(y == expected++);
      },
      {"x", "y"});
   ...
   gSystem->Unlink((std::string("_out") + fnamePrefix + ".root").c_str());
}
