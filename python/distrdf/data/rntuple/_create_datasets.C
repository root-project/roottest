...
void create_check_backend()
{
    ...
    std::vector<std::string> filenames{
        "distrdf_roottest_check_backend_0.root",
        "distrdf_roottest_check_backend_1.root",
        "distrdf_roottest_check_backend_2.root"};

    for (auto i = 0; i < ntpl_names.size(); i++)
    {
        auto model = RNTupleModel::Create();
        auto fldX = model->MakeField<int>("x", 1);
        auto ntpl = RNTupleWriter::Recreate(std::move(model), ntpl_names[i], filenames[i]);
...
void create_cloned_actions()
{
    std::vector<std::uint64_t> clusters{
        66, 976, 1542, 1630, 2477, 3566, 4425, 4980, 5109, 5381, 5863, 6533, 6590, 6906, 8312, 8361, 8900, 8952, 9144, 9676};
    ...
    auto ntpl = RNTupleWriter::Recreate(std::move(model), datasetname, filename);
...
{
    auto ntpl = RNTupleWriter::Recreate(RNTupleModel::Create(), "empty", "empty.root");
}

void create_definepersample()
{
    std::vector<std::string> filenames{
        "distrdf_roottest_definepersample_sample1.root",
...
void create_friend_trees_alignment()
{
    std::vector<std::string> ntpl_names{
        "distrdf_roottest_check_friend_trees_alignment_1",
        "distrdf_roottest_check_friend_trees_alignment_2",
        "distrdf_roottest_check_friend_trees_alignment_3",
        "distrdf_roottest_check_friend_trees_alignment_4",
        "distrdf_roottest_check_friend_trees_alignment_5",
        "distrdf_roottest_check_friend_trees_alignment_6"};
    std::vector<std::string> filenames{
        "distrdf_roottest_check_friend_trees_alignment_1.root",
        "distrdf_roottest_check_friend_trees_alignment_2.root",
        "distrdf_roottest_check_friend_trees_alignment_3.root",
        "distrdf_roottest_check_friend_trees_alignment_4.root",
        "distrdf_roottest_check_friend_trees_alignment_5.root",
        "distrdf_roottest_check_friend_trees_alignment_6.root"};
...
void create_friend_trees()
{
    auto create_ntpl = [](const std::string &ntplname, const std::string &filename, int gaus_mean)
    {
        ...
        auto ntpl = RNTupleWriter::Recreate(std::move(model), ntplname, filename);
        TRandom r;
        for (auto i = 0; i < 10000; i++)
        {
    ...
    TFile out_file{filename_7584.c_str(), "recreate"};
    {
        auto model = RNTupleModel::Create();
        auto fldX = model->MakeField<double>("x");
        auto ntpl = RNTupleWriter::Append(std::move(model), ntpl_name_rn1, out_file);
        TRandom r;
        for (auto i = 0; i < 10000; i++)
        {
    ...
    }
    {
        auto model = RNTupleModel::Create();
        auto fldX = model->MakeField<double>("x");
        auto ntpl = RNTupleWriter::Append(std::move(model), ntpl_name_rn2, out_file);
        TRandom r;
        for (auto i = 0; i < 10000; i++)
        {
...
void create_reducer_merge()
{
    ...
    auto ntpl = RNTupleWriter::Recreate(std::move(model), ntpl_name, filename);
...
void create_rungraphs()
{
    ...
    auto model = RNTupleModel::Create();
    auto fldb1 = model->MakeField<int>("b1", 42);
    auto fldb2 = model->MakeField<int>("b2", 42);
    auto fldb3 = model->MakeField<int>("b3", 42);
    auto ntpl = RNTupleWriter::Recreate(std::move(model), ntpl_name, filename);
    auto nentries = 10000;
